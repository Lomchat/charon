 ...
 * @returns {string[]}
 */
function buildSshArgs(vps, opts, remoteCommand) {
...
/**
 * Build a dedicated SSH command that writes one exact file range to stdout.
 * Paths and the stat precondition are base64url rather than shell-quoted: the
 * resulting alphabet is inert in a POSIX remote shell, including for paths
 * containing quotes, newlines, `$()` or leading dashes.
 *
 * @param {{ip: string, sshUser: string, sshPort: number|string}} vps
 * @param {{root: string, path: string, offset: number, length: number, expectedVersion?: string|null}} spec
 * @param {{keyPath?: string|null, serverAliveInterval?: number, serverAliveCountMax?: number}} [opts]
 */
function buildAgentFileStreamSshArgs(vps, spec, opts = {}) {
  if (!Number.isSafeInteger(spec.offset) || spec.offset < 0 ||
      !Number.isSafeInteger(spec.length) || spec.length < 0) {
    throw new TypeError('file stream offset/length must be non-negative safe integers');
  }
  const encode = (value) => Buffer.from(String(value), 'utf8').toString('base64url');
  const expected = spec.expectedVersion == null
    ? ''
    : ` --expected-version ${encode(spec.expectedVersion)}`;
  const command =
    `exec ${agentHomeEnvPrefix()}${REMOTE_PY} ${REMOTE_AGENT_PATH}` +
    ` --stream-file ${encode(spec.root)} ${encode(spec.path)}` +
    ` --offset ${spec.offset} --length ${spec.length}${expected}`;
  return buildSshArgs(vps, opts, command);
}

/**
 * Build the sibling byte channel that creates a directory ZIP on stdout.
 * The archive has no temporary remote file and therefore no size/range
 * arguments; the same base64url path rule keeps the remote shell inert.
 *
 * @param {{ip: string, sshUser: string, sshPort: number|string}} vps
 * @param {{root: string, path: string}} spec
 * @param {{keyPath?: string|null, serverAliveInterval?: number, serverAliveCountMax?: number}} [opts]
 */
function buildAgentZipStreamSshArgs(vps, spec, opts = {}) {
  const encode = (value) => Buffer.from(String(value), 'utf8').toString('base64url');
  const command =
    `exec ${agentHomeEnvPrefix()}${REMOTE_PY} ${REMOTE_AGENT_PATH}` +
    ` --stream-zip ${encode(spec.root)} ${encode(spec.path)}`;
  return buildSshArgs(vps, opts, command);
}

module.exports = {
  REMOTE_AGENT_PATH, REMOTE_PY, KNOWN_HOSTS_PATH,
  buildAgentSshArgs, buildAgentFileStreamSshArgs, buildAgentZipStreamSshArgs,
  controlPath,
};
