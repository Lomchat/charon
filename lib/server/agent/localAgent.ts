...
// Helpers pour gérer l'agent local — celui qui tourne sur la machine du
// dashboard (pas sur un VPS distant). Aucun SSH ici, tout est local.
//
// Layout assumé :
//   ~/.charon/charon-agent.pyz   ← déployé
//   ~/.charon/venv/bin/python    ← interpréteur du venv (créé au bootstrap)
//   ~/.config/systemd/user/charon-agent.service  ← unit
//
// Status :
//   - sha du pyz local : computeFileSha12()
...
export type LocalAgentStatus = {
  ...
  outOfDate: boolean;
  serviceActive: boolean | null;  // null = systemctl pas dispo / erreur
};
...
// Copie le pyz buildé sur le pyz déployé, puis restart systemd-user. Si le
// service n'existe pas, on tente nohup. Renvoie le sha post-deploy.
export async function updateLocalAgent(): Promise<LocalUpdateResult> {
  try {
    if (!fs.existsSync(DASHBOARD_PYZ)) {
      return { ok: false, detail: `pyz buildé introuvable: ${DASHBOARD_PYZ}` };
    }
    // S'assure que ~/.charon existe
    fs.mkdirSync(path.dirname(LOCAL_PYZ), { recursive: true });
    // Atomic : copyFileSync vers .new puis rename
  ...
  // Restart systemd-user. Si ça foire, fallback nohup pour ne pas laisser
  // le user avec un agent à jour mais arrêté.
  const r = await execLocal('systemctl', ['--user', 'restart', 'charon-agent.service'], 15000);
  let serviceActive: boolean;
  if (r.code === 0) {
    // Vérifie qu'il est bien actif
    ...
    serviceActive = false;  // pas via systemd
  }
  ...
  return {
    ...
    serviceActive,
    detail: `agent local mis à jour${newPyzSha ? ` (${newPyzSha})` : ''}`,
  };
}
