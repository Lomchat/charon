...
export async function GET() {
  ...
  return NextResponse.json({ endpoints: await Promise.all(db.select().from(customEndpoints).all().map(async (r) => publicEndpoint(await enrichEndpoint({ ...JSON.parse(r.config), id: r.id })))) });
}
export async function POST(req: Request) {
  ...
  try {
    ...
    const endpoint = await enrichEndpoint(withEndpointChecks(parseEndpoint(body.endpoint, previous), String(body.vpsId || '')));
