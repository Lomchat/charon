...
// Renvoie la liste des sessions Claude existantes sur le VPS
// (fichiers .jsonl dans ~/.claude/projects/).
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Python qui scanne ~/.claude/projects et émet un seul JSON.
  const py = [
    "import os, json, sys",
  ...
  // Choisit le bon python (3.10+ recommande, mais le scan marche en 3.9 aussi)
  const cmd =
    `PY=$(command -v python3.13 || command -v python3.12 || command -v python3.11 || command -v python3.10 || command -v python3); ` +
    `"$PY" -c "${py.replace(/"/g, '\\"')}"`;
  const r = await sshExec(v, cmd, { timeoutMs: 30_000 });
  if (!r.ok) {
    return NextResponse.json({ error: 'ssh failed', stderr: r.stderr.slice(-400) }, { status: 500 });
  }
  let parsed: any[] = [];
